...
<div class="max-w-screen-2xl mx-auto">
    <div class="mb-6">
        <h1 class="text-3xl font-bold text-slate-900">Edit Role Privileges</h1>
        <p class="mt-1 text-sm text-slate-500">
            Update permissions for <span class="font-semibold">{{ $role->name }}</span>.
        </p>
    </div>

    <form action="{{ route('role-privileges.update', $role->id) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            @php
                $formPermissions = old('permissions', $selectedPermissions);
            @endphp
                                ...
                                @foreach($permissions as $permission)
                                    <td class="border border-slate-200 px-4 py-4 text-center">
                                        <input type="checkbox"
                                               ...
                                               @checked(in_array($permission->id, $formPermissions[$page->id] ?? []))>
                                    </td>
                                @endforeach
                            </tr>
                        @endforeach
